...
\begin{figure}[H]
    \centering
    \small
    % Pipeline global : classification -> décision -> regression -> affichage
...
% \section{Résultats expérimentaux (synthèse)}

% Cette sous-section regroupe les résultats obtenus lors des expériences préliminaires et des entraînements décrits dans le rapport d'entraînement que vous avez fourni.

% \subsection{Classification}

% \begin{itemize}
%     \item \textbf{Précision validation} : $\approx 97{,}6\%$ (arrêt précoce à 6 epochs faute de GPU, résultats à confirmer avec entraînement complet).
%     \item \textbf{Métriques} : matrice de confusion, F1-score macro (valeur élevée), recall et precision par classe montrant que la plupart des classes sont bien séparées ; certaines confusions mineures entre variétés visuellement proches.
%     \item \textbf{Export} : modèle Keras exporté puis converti en TFLite avec quantification post-training ; modèle utilisable sur device avec bonne latence.
% \end{itemize}

% \subsection{Régression}

% \begin{itemize}
%     \item \textbf{Modèle final} : CNN personnalisé entraîné from-scratch (cf. rapport). Performance acceptable avec MAE/RMSE compétitifs après sélection.
%     \item \textbf{Quantification} : conversion TFLite float16 aboutit à un bon compromis taille/précision ; PTQ int8 testé mais parfois induit légère dégradation (corrigée par QAT).
%     \item \textbf{Observations} : variétés à forte variance (ex. \texttt{Macc55}) restent difficiles ; une stratégie d'entraînement par-varieté ou un embedding de variété peut améliorer la précision.
% \end{itemize}

% \section{Tableau comparatif : classification vs régression}

% \begin{table}[H]
%     \centering
%     \caption{Comparaison synthétique des pipelines classification et régression.}
%     \label{tab:comparaison}
%     \begin{tabular}{@{}p{3.6cm}p{5.2cm}p{5.2cm}@{}}
%         \toprule
%         \textbf{Aspect} & \textbf{Classification (\(\mathcal{D}_{\text{clf}}\))} & \textbf{Régression (\(\mathcal{D}_{\text{reg}}\))} \\ \midrule
%         Objectif & Déterminer si image = haricot + identifier variété (9 classes) & Estimer $T_c$ en minutes (régression continue) \\
%         Jeu de données & ≈ (9 classes : 8 variétés + autre) ; classe 'autre' sur-représentée (~800 images) & 11 200 images ; 8 variétés ; 1400 images / variété \\
%         Prétraitement & Resize 224×224, normalisation ImageNet, augmentations agressives & Resize 224×224, recadrage centré si besoin, normalisation canaux, min-max target scaling \\
%         Modèles testés & MobileNetV2 (fine-tune), CNN léger & CNN personnalisé (final), EfficientNet-B0 (testé) \\
%         Quantification cible & INT8 (PTQ avec calibration ; QAT si nécessaire) & Float16 (PTQ) ; int8 testé si acceptable \\
%         Loss & Categorical CE (class weights) & MSE (MAE/MAPE pour suivi) \\
%         Métriques principales & Acc, F1-macro, Recall/Precision, matrice de confusion & MAE, RMSE, $R^2$, erreur relative (MAPE) \\
%         Contraintes & Latence très faible, robustesse hors-domaine & Compromis précision / taille ; sensibilité à la variance inter-variétés \\
%         Décision dans l'application & Filtre : si \texttt{autre} $\Rightarrow$ refuser prédiction de $T_c$ & Exécuté uniquement si l'entrée validée comme haricot \\
%         \bottomrule
%     \end{tabular}
% \end{table}
