\documentclass[]{settings/wi-thesis}
\addbibresource{library.bib}

\begin{document}
\titleThesis{TinyML for Bean Cooking Time Prediction}

\author{MBAIGOLMEM DANG-DANG THIERY \thanks{dev.thiery.dangdang@gmail.com}}

% % Matriculation Number
% \id{20A179FS}

% % Field of Study
% \field{Ing\'enerie Informatique}

% % Address, for declarations at the end of the thesis.
% \city{Université de Ngaoundéré}

% \professor{Prof. Dr. Ing. FENDJI JEAN LOUIS KEDIENG EBONGUE}

% \supervisor{Supervisor, Pr Dr BOUKAR}

% Submission Date
% \date{08.09.2025}

\maketitle
\tableofcontents
\listoffigures
\listoftables

\input{content/01-introduction} %% Ok Ici
\input{content/review} 			%% Ok Ici
\input{content/02-chapter1}		%% Ok Ici
\input{content/03-chapter2}		%% Ok Ici
\input{content/04-chapter3}		%% Ok Ici
\input{content/10-conclusion} %% <- Probleme

\begin{appendix}
	\input{content/99-appendix}
\end{appendix}

\begin{backmatter}

	\begingroup
	\setlength{\emergencystretch}{.5em}

	\printbibliography[
		heading=bibintoc,
		title={Bibliographie}
	]
	\endgroup
	\frenchDeclaration

\end{backmatter}

\end{document}
